...
class base(object):
    ...
    def apply_mute(self, traces):
        ...
        if 'MuteBodyWaves' in PAR.MUTE:
            traces = signal.mute_body_waves(traces,
                ...
                PAR.MUTE_BODY_WAVES_CONST1, # (units: time)
                PAR.MUTE_BODY_WAVES_SLOPE2, # (units: time/distance)
                PAR.MUTE_BODY_WAVES_CONST2, # (units: time)
                self.get_time_scheme(traces),
                self.get_source_coords(traces),
                self.get_receiver_coords(traces))
    ...
    def check_mute(self):
        ...
        if 'MuteBodyWaves' in PAR.MUTE:
            assert 'MUTE_BODY_WAVES_SLOPE1' in PAR
            assert 'MUTE_BODY_WAVES_CONST1' in PAR
            assert 'MUTE_BODY_WAVES_SLOPE2' in PAR
            assert 'MUTE_BODY_WAVES_CONST2' in PAR
            assert PAR.MUTE_BODY_WAVES_SLOPE1 >= 0.
            assert PAR.MUTE_BODY_WAVES_SLOPE2 >= 0.
